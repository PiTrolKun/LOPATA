...
public sealed class LiteraryWorkIndex(LiteraryProjectLayout layout, ILiterarySourceEmbedding? embedding = null)
{
    ...
    public static LiteraryWorkManifest? Current(LiteraryProjectLayout layout, LiterarySource source, string text)
    {
        ...
        return value is not null && value.PartId == source.Id && value.Revision == Revision(text)
            && value.ModelRevision == GigaEmbeddingInstallation.Revision + ImportEligibility.Fingerprint(layout.Root, source.Id) ? value : null;
    }
    public async Task PrepareAsync(IProgress<LiteraryPreparationProgress> progress, CancellationToken ct)
    {
        ...
        try
        {
            foreach (var source in snapshot.Sources.Where(s => s.Id != snapshot.ActiveId))
            {
                ...
                var eligibility = ImportEligibility.Fingerprint(layout.Root, source.Id);
                var allowed = ImportEligibility.AllowedSpans(layout.Root, source.Id, text);
                if (allowed.Count == 0) continue;
                var folder = layout.EnsureFolder(Path.Combine("Rag", "Work", source.Id));
                var staging = Path.Combine(folder, revision + (eligibility.Length > 0 ? "-" + ImportSession.Hash(eligibility)[..12] : "")); Directory.CreateDirectory(staging);
                var input = Path.Combine(staging, "text.json"); var output = Path.Combine(staging, "vectors.jsonl");
                LiteraryChapterFiles.Write(input, JsonSerializer.Serialize(allowed.Select(segment => new { source = source.Id, section = source.Number,
                    text = segment.Text, offset = text[..segment.Start].EnumerateRunes().Count(), kind = "project" })));
                ...
                layout.EnsurePresent();
                if (Revision(LiteraryChapterFiles.Read(path)) != revision || ImportEligibility.Fingerprint(layout.Root, source.Id) != eligibility) throw new IOException("The fixed text changed during indexing.");
                var collection = Convert.ToHexString(SHA256.HashData(Encoding.UTF8.GetBytes(source.Id + revision + eligibility)))[..32].ToLowerInvariant();
                ...
                layout.EnsurePresent();
                LiteraryChapterFiles.Write(manifestPath, JsonSerializer.Serialize(new LiteraryWorkManifest(source.Id, revision, collection, points, GigaEmbeddingInstallation.Revision + eligibility)));
