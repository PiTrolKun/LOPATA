...
function global:gh {
    ...
    if ($line -match '^api repos/.+/releases/53$') {
        $assets = if($global:UpdatePublishUploaded) { @(
            @{name='LOPATA_Setup_0.1.53-beta.exe';size=4;digest=('sha256:' + $(if($global:UpdatePublishBadHash){'b'*64}else{$global:UpdatePublishHash}))},
            @{name='lopata-update.json';digest=('sha256:'+(Get-FileHash -LiteralPath (Join-Path $scratch 'release-0.1.53-beta/lopata-update.json')).Hash.ToLowerInvariant())}) } else { @() }
        return @{ id=53; tag_name='v0.1.53-beta'; draft=$true; prerelease=$true; assets=@($assets) } | ConvertTo-Json -Depth 5
    }
    if ($line -match '^api repos/.+/releases\?') {
        ...
        $lastPage = @($items[2..3])
        if($global:UpdatePublishResume) { $lastPage += @{ id=53; tag_name='v0.1.53-beta'; draft=$true; prerelease=$true; target_commitish=('a'*40); assets=@() } }
        return ConvertTo-Json -InputObject @(@($items[0..1]), $lastPage) -Depth 6
    }
}
try {
    ...
    if ($commands -notmatch 'release create .+--draft --prerelease' -or $commands -notmatch 'release edit .+--draft=false' -or $commands -notmatch 'release upload') { throw 'Draft workflow missing.' }
    ...
    $global:UpdatePublishCommands.Clear(); $global:UpdatePublishBadHash=$false; $global:UpdatePublishResume=$true
