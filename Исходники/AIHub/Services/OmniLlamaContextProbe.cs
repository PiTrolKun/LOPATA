...
public static class OmniLlamaContextProbe
{
    public static async Task<int> MeasureAsync(HttpClient http, Uri server,
        IReadOnlyList<ImageAnalysisHiddenMessage> conversation, string imageDataUrl, CancellationToken token)
    {
        ...
        using var template = await PostAsync(http, new Uri(server, "apply-template"),
            OmniLlamaProtocol.BuildRequest(conversation, imageDataUrl), token).ConfigureAwait(false);
