...
## 2026-09-09 — формат рабочих глав, 0.1.82-dev

Новых зависимостей нет. Для DOCX используется существующий DocumentFormat.OpenXml 3.5.1. В проектах литературной мастерской добавлена схема chapters/index.json версии 1: числовые номера глав/частей, устойчивые ID, активная часть, название, завершение и интервал автосохранения. Тексты — отдельные нумерованные UTF-8 TXT; резерв .bak и журнал pending.json защищают запись и создание частей. working-draft.txt импортируется однократно и сохраняется как миграционный резерв. Старые имена после переименования сохраняются в recovery; читательский экспорт использует только зарегистрированные файлы. Контракты project.json, моделей и чатов не менялись. Детали: ../Тесты/LiteraryEditor/README.md.


## 2026-09-09 — временный backend для исследования циклов

Итог: исследование завершено, временная копия b9000 вместе с архивом и скопированными CUDA DLL удалена 2026-09-09. Новых runtime-зависимостей и моделей в приложение не добавлено. Воспроизводимые Python-инструменты и результаты сохранены в стенде; подтверждение очистки — `Тесты/Runeweaver/loop_lab_20260909/cleanup.json`. Следующий абзац описывает происхождение временной загрузки.

По разрешению пользователя на дополнительный софт скачан официальный llama.cpp b9000 (1a03cf47f), Windows CUDA 12.4, только для изолированного сравнения. Источник: https://github.com/ggml-org/llama.cpp/releases/tag/b9000; архив llama-b9000-bin-win-cuda-12.4-x64.zip, 214359395 байт, SHA256 5fc0ce2cdd5b626edd057441fcf0ac3a9982d92edc442747eff651dcb7ffedc0. Временный путь `_tmp/loop_backend_b9000_20260909`; три CUDA DLL скопированы из существующего b9442, исходники не изменены. llama.cpp — MIT; CUDA-компоненты имеют отдельные условия NVIDIA. Это исследовательский инструмент, не новая зависимость приложения и не часть установщика. После исследования временная копия подлежит удалению; параметры, версия и результаты остаются в `Тесты/Runeweaver/loop_lab_20260909`.

## 2026-09-09 — экспериментальная Runeweaver в приложении

0.1.79-dev: литературные ИИ1/ИИ2 используют один внешний `MN-12B-Runeweaver-RP-RU.Q4_K_M.gguf`, ранее скачанный со стенда. Источник limloop/MN-12B-Runeweaver-RP-RU-GGUF, revision84fa96954eef3eec4e92433e133c5bb1c774fc22,7477218976байт,SHA25675b3107399b2f813b7ae0b98758e65aa5694b4de38b7576636e661c98506a565. Путь в локальном literary-model.json; в код не зашит. Файл не копируется в установщик и не раздаётся как управляемый компонент. Лицензионный аудит поставки остаётся отдельным этапом, сведения предыдущего стенда сохранены. Backend — существующий llama.cpp b9442, CUDA, родной GGUF ChatML, unified KV24576,2слота, приложение ограничивает роли8192/16384 и одну генерацию. Новых пакетов нет. [Проверка и ограничения](../Тесты/Runeweaver/app_smoke_20260909/README.md).

## 2026-09-07 — обновления приложения

0.1.53-dev / публичная 0.1.53-beta: новые runtime-зависимости не добавлены. GitHub Releases REST используется для публичных метаданных/установщиков; ключи клиенту не нужны. Скачивание через существующий SegmentedModelFileDownloader, общая настройка соединений. GitHub CLI остаётся внешним инструментом публикации, не поставляется с приложением. Inno Setup получил ожидание завершающегося процесса /WAITFORPID; согласия и каталог лицензий прежние. [Порядок сборки/публикации](ОБНОВЛЕНИЯ.md).
...
## 2026-09-07 — ввод и хранение изображений, 0.1.51-dev

Новых внешних runtime-зависимостей нет: WPF DataObject/Clipboard/WIC, .NET HttpClient/FileStream и уже используемый System.Drawing для bitmap. ImageTransferReader/ImageInputService/ImageAssetStore — собственные компоненты общего ввода. Лицензионный каталог/согласия, модели и backends не менялись. Сетевое получение только по действию пользователя, без браузерных cookies; проверка действительного формата, 32 МиБ/40 млн пикселей/30 с/5 редиректов. Пользовательские картинки и результаты не входят в поставку; Git исключает AI_HUB/Images. [Проверки и ограничения](../Тесты/Image_input_20260907/README.md).
...
## 2026-09-07 — Гамма Empero Ridge 27B, 0.1.50-dev

model-qwen3-8-27b-ridge-37bpw: [Empero GGUF](https://huggingface.co/empero-ai/Qwen3.8-27B-Ridge-GGUF/tree/486faa5f2032ff99bdc8993ade1b8fff13d1464c), Qwen3.8-27B-Ridge-3.7bpw.gguf (12599187008 байт) и mmproj-Qwen3.8-27B-BF16.gguf (931145952 байт). Оба SHA-256 и отдельный каталог закреплены в ManagedModelCatalog.Gamma.cs. Это смешанная квантизация Ridge, не Distill. База Qwen/Qwen3.8-27B, архитектура GGUF qwen35. Проверка 2026-09-06: Apache-2.0 по карточке и [оригинальному LICENSE базы](https://huggingface.co/Qwen/Qwen3.8-27B/blob/1d4bf0f2ff6012fd82039f2fa52739d0dd7c60c0/LICENSE); запись включена в штатный каталог согласий. Веса отдельно скачивает пользователь, они не входят в установщик.

Backend прежний llama.cpp b9442, один слот/32768 токенов, GPU модель/проектор; Kokoro CPU. Thinking включён, temperature 1.0, top_p .95, top_k 20; Альфа/Бета сохраняют .6. Отдельное speculative decoding/MTP не включено. Старые Qwen2.5 файлы и результаты сохранены, продолжение старых работ новой моделью блокируется. Получены только метаданные и короткие GGUF-заголовки; загрузка полных весов, совместимость всех тензоров, качество и полный пик памяти ещё не проверены. Подробности: Диагностика/2026-09-06_Gamma_Ridge/README.md. Установщик не пересобирался; базовые условия семантически прежние.

## 2026-09-06 — принята Бета Empero 9B, 0.1.49-dev

model-qwen3-8-9b-distill-q4km: [Empero GGUF](https://huggingface.co/empero-ai/Qwen3.8-9B-Distill-GGUF/tree/760121cd70bb4c36b2b5ec58eb765e0df5987efe), Qwen3.8-9B-Q4_K_M.gguf, 5780090176 байт; [Unsloth базовый проектор Qwen3.5-9B](https://huggingface.co/unsloth/Qwen3.5-9B-GGUF/tree/3885219b6810b007914f3a7950a8d1b469d598a5), mmproj-F16.gguf, 918166080 байт. Оба SHA-256 закреплены в ManagedModelCatalog.Beta.cs. Apache-2.0 по карточкам источников, проверка 2026-09-06; запись в штатном лицензионном каталоге с двумя источниками. Получение пользователем отдельно через штатное многопоточное скачивание/согласие, веса с программой не распространяются. Это составная пара, не доказанная конверсия визуальных весов Empero; три реальные пользовательские работы подтвердили практическую совместимость. Качество положительно оценено пользователем.

Runtime llama.cpp b9442 прежний, qwen35, GPU модель/проектор, Kokoro CPU. 32768 токенов, один слот, прежние параметры Альфы; новая диагностическая подпись Беты qwen35-q4km-f16. Прежний Kimi/Florence/редактор не входят в новые работы Беты; старые результаты и файлы сохраняются. Установщик не пересобирался; список его базовых условий семантически прежний. Полный пик 32/10 не подтверждён; подробности в Тесты/Beta_user_20260906/README.md.

## 2026-09-06 — политика контекста и совместимость данных, 0.1.48-dev

Зависимости/модели/лицензии не изменены. Альфа llama.cpp b9442: старый default Qwen3VL --image-max-tokens 4096 закреплён явно; /apply-template и /tokenize используются для безопасной оценки входа с визуальным верхним пределом. Альфа/Гамма проверяют 95% рабочего окна и резерв 4096 токенов ответа; генерация ограничена оставшимся безопасным бюджетом. ContextBlocked — добавочное bool-поле session.json (schema 3, старые файлы по умолчанию false), старые версии игнорируют незнакомое поле. Сохранённые данные не мигрируются разрушительно. Отчёт и источники b9442: Тесты/Omni_guard_20260906/README.md. Кэш RAM не ограничивался заново; гарантии полного пика 16/8 по этой проверке нет.

## 2026-09-06 — новая модель Альфы, 0.1.47-dev

По решению пользователя Qwen2.5-Omni-3B GGUF заменяется на `empero-ai/Qwen3.8-4B-Distill`. Название принадлежит стороннему автору; архитектура исходника — Qwen3.5 (`qwen35`). Подробная проверка метаданных, vision-тензоров и совместимости: [отчёт](Диагностика/2026-09-06_Empero_Qwen38_4B/README.md).

Проверено 2026-09-06: [исходник](https://huggingface.co/empero-ai/Qwen3.8-4B-Distill/tree/c83cb7aa2999d2f35c43e9ae0634a30eb8985a1e), [GGUF-комплект mradermacher](https://huggingface.co/mradermacher/Qwen3.8-4B-Distill-GGUF/tree/796f0c8fbdab2e6e0c14a13d499075850e4a16b3).

- ID компонента: `model-qwen3-8-4b-distill-q5km`; optional download через общий лицензионный gate.
- Ревизия GGUF: `796f0c8fbdab2e6e0c14a13d499075850e4a16b3`.
...
- Сумма: 3 833 849 728 байт; отдельная папка `Vision/Qwen3.8-4B-Distill-GGUF/796f0c8fbdab2` внутри выбранного хранилища.

Карточки исходника и конверсии заявляют Apache-2.0. Отдельные LICENSE-файлы в этих двух репозиториях на дату проверки не найдены; приложен стандартный текст Apache-2.0. Распространение допускается на условиях этой лицензии с необходимыми уведомлениями, однако веса в установку ЛОПАТЫ не включаются и скачиваются пользователем отдельно. Старое согласие Qwen Research не засчитывается за новую модель; прежняя запись лицензии сохранена для истории. Каталог подготовлен штатным скриптом; Terms всех прежних компонентов, текст базового согласия и шаблон installer receipt сохранены без изменения содержания.

Runtime прежний: llama.cpp b9442 win-cuda-12.4-x64, MIT; общие backend-файлы не заменялись, новых пакетов нет. Профиль: 32768 токенов, один слот, GPU model/projector, fit off, no-context-shift, offline. Sampling: temperature 0.6, top_p 0.95, top_k 20, min_p 0. Thinking включён шаблоном; reasoning хранится в сыром SSE, конечный ответ отдельно передаётся в UI и следующую реплику. Runtime ID `qwen35_llama_cpp`, версия Alpha pipeline 2. Старые Alpha-работы сохраняют прежнюю модель и доступны для чтения/экспорта; новая генерация требует новой работы.

Kokoro CPU и общий двухэтапный сценарий сохранены. Цель 16 ГБ RAM / 8 ГБ VRAM предварительная; реальное качество русского/vision и полный пик памяти не измерены. Схема Беты «визуальный черновик → редактура ядром» записана как резерв при подтверждённой языковой проблеме, без подключения второго исполнителя сейчас.

## 2026-09-06 — Alpha GGUF, 0.1.46-dev

Проверено 2026-09-06 по метаданным [GGUF-репозитория](https://huggingface.co/ggml-org/Qwen2.5-Omni-3B-GGUF/tree/75f1b73b657a50f5092502799457ccb4a4a1f9df):
- revision: 75f1b73b657a50f5092502799457ccb4a4a1f9df;
- Qwen2.5-Omni-3B-Q4_K_M.gguf: 2 104 931 648 байт; SHA-256 4b0bd358c1e9ec55dd3055ef6d71c958c821533d85916a10cfa89c4552a86e29;
- mmproj-Qwen2.5-Omni-3B-Q8_0.gguf: 1 538 031 328 байт; SHA-256 4e6c816cd33f7298d07cb780c136a396631e50e62f6501660271f8c6e302e565.

[API метаданных](https://huggingface.co/api/models/ggml-org/Qwen2.5-Omni-3B-GGUF?blobs=true) связывает конверсию с [Qwen Research License исходной модели](https://huggingface.co/Qwen/Qwen2.5-Omni-3B/blob/main/LICENSE). Квантование не меняет условия. Использован уже сохранённый текст исходной лицензии; отдельное новое разрешение на коммерческое использование не установлено. Полнота нативных notices остаётся прежним открытым пунктом.
В каталоге лицензий отдельный ID model-qwen2-5-omni-3b-q4km, optional download, проверка через существующий gate. Согласие базовой поставки и установщик не расширялись весами.
Runtime: существующий llama.cpp b9442 win-cuda-12.4-x64, лицензия MIT по прежнему каталогу. [Документация этой версии](https://github.com/ggml-org/llama.cpp/blob/b9442/docs/multimodal.md) перечисляет Qwen2.5-Omni-3B; локальный --help подтверждает необходимые флаги. Реальный прогон GGUF пока не выполнен.
Новый адаптер проекта OmniLlamaRuntimeService запускает отдельный процесс штатного backend на свободном loopback-порту и завершает только собственный процесс. Профиль 32768 токенов, один слот, GPU model/projector, fit off, no-context-shift, offline, SSE. Новых библиотек/системных пакетов нет. Kokoro CPU прежняя.
Цель 16 ГБ RAM / 8 ГБ VRAM не является результатом замера. Пики и скорость будут записаны после скачивания пользователем.
...
Новых моделей и зависимостей нет. Используется прежний Empero Qwen3.8-9B-Distill / OmniLlamaProfile.Beta для анализа и оформления. В batch.json добавлен BundleId; отсутствующее поле старого формата означает light (Альфа). Schema=1 сохранена: это обратно совместимое добавление. Лицензионный каталог, загрузчик и установщик не менялись.


## 2026-09-08 — паспорт литературного проекта (внутренний формат)
В 0.1.69-dev добавлен project.json, SchemaVersion=1: ID, дата создания, название проекта/произведения, автор, язык, форма, жанры, замысел, ограничения, первоисточник, относительные пути копий материалов и культурные ориентиры. Каталог %LocalAppData%/AI_HUB/Literary/projects.json хранит пути и ActiveId. Это формат собственных метаданных, не внешний компонент и не формат экспорта произведения. Сериализация — уже используемый System.Text.Json; новые зависимости не добавлены. Список стран сформирован из культур .NET, локализованные подписи подготовлены встроенным Intl.DisplayNames Node и включены в RU/EN словари; Node для работы приложения не требуется. Материалы пока только локальные копии, без индексации и сетевых запросов.

## 2026-09-08 — экспериментальный Писатель CPU

ИИ1 литературного экрана переиспользует зарегистрированный артефакт Альфы Qwen3.8-4B-Distill Q5_K_M (empero-ai, GGUF mradermacher) и существующий llama.cpp b9442. Новый компонент не скачивается и не распространяется; проектор для текстового чата не загружается. Отдельный CPU процесс, 32768 контекст, до 8 потоков, дополнительный RAM prompt cache отключён. Условия лицензии существующего артефакта не изменены; расширение лицензионного UX отложено по указанию пользователя. Сценарий экспериментальный, ИИ2 ещё не подключён.

## 2026-09-08 — экспериментальный Советник GPU

Тот же артефакт Альфы и llama.cpp b9442 используются вторым независимым процессом для ИИ2: CUDA0, ngl 99, 32768 контекст, cache-ram 0, без проектора. Это подключение уже установленного компонента, не новая загрузка/зависимость. ИИ1 остаётся CPU. Лицензионный UX и публикации остаются отложенными по указанию пользователя.

## 2026-09-09 — экспериментальная модель Runeweaver

limloop/MN-12B-Runeweaver-RP-RU-GGUF, Q4_K_M, ревизия 84fa96954eef3eec4e92433e133c5bb1c774fc22. Источник: https://huggingface.co/limloop/MN-12B-Runeweaver-RP-RU-GGUF . Карточка указывает Apache-2.0; происхождение merge и условия поставки требуют отдельной проверки перед внедрением. Сейчас внешняя экспериментальная модель, не управляемый компонент, не входит в установщик и не распространяется проектом. Стенд использует существующие Python 3.12 и llama.cpp b9442; новых пакетов нет. Размер/SHA256 закреплены в Инструменты/Runeweaver/config.json.

Runeweaver, проверка2026-09-09: b9442 обслуживает два независимых слота по8192 на одних весах при последовательных запросах; пик выделенной VRAM9.807GiB. Шаблон сервераChatML совпадает с upstream chat_template.jinja. Модель остаётся внешним экспериментом, не включена в поставку. Результаты: Тесты/Runeweaver/suite_20260909/README.md.

## 2026-09-09 — чтение литературного проекта, 0.1.84-dev
Новых компонентов/зависимостей нет. Схемы project.json и chapters/index.json не изменены. Временный контекст запроса включает ID проекта/части, hash текста, полный снимок редактора и прочитанные фрагменты. Протокол чтения JSON schema исполняется существующим llama.cpp; это программный диспетчер, не заявление о native function calling Runeweaver. Список/поиск/чтение ограничены зарегистрированными TXT, запись недоступна. Политика публикации модели/установщика прежняя. Отчёт: Тесты/LiteraryReading/README.md.
