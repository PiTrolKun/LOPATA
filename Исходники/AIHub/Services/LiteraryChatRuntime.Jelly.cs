...
public sealed partial class LiteraryChatRuntime
{
    public async Task<string> ExtractJellyAsync(string source, CancellationToken token)
    {
        ...
        try
        {
            ...
            using var template = await PostJsonAsync("apply-template", new { messages = messages.Select(m => new { role = m.Role, content = m.Content }), add_generation_prompt = true,
                chat_template_kwargs = LiteraryModelPolicy.Thinking(false) }, ct);
            var count = await TokenCountAsync(template.RootElement.GetProperty("prompt").GetString()!, true, ct);
            await AvailableReplyAsync(count, ct);
            var result = await LiteraryLoopRecovery.RunAsync(async recovery =>
            {
                await PrepareAsync(ct).ConfigureAwait(false);
                var reply = await AvailableReplyAsync(count, ct);
                var body = JsonSerializer.Serialize(new { messages = messages.Select(m => new { role = m.Role, content = m.Content }),
                    id_slot = LiteraryModelPolicy.Slot(LiteraryChatProfile.Writer), max_tokens = reply, temperature = 0.1,
            ...
            diagnostics.Write("jelly_raw", result); return LiteraryStructuredReply.Json(result);
        }
