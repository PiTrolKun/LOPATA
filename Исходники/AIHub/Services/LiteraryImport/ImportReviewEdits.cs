...
{
    /// <summary>Persist an edit without silently approving unresolved import markings.</summary>
    public static void SaveDraft(string root, string id, string before, string after)
    {
        if (before == after) return;
        if (after.Length > LiteraryModelPolicy.DraftCharacters)
            throw new InvalidDataException("Literary.Import.EditTooLong");
        var layout = new LiteraryProjectLayout(root); layout.EnsurePresent();
        using var lease = new FileStream(Path.Combine(root, "Import", "review.lock"),
        ...
        if (LiteraryChapterFiles.Read(Path.Combine(root, "chapters", part.FileName)) != before)
        ...
        var doubts = new[] { new ImportSpan(0, after.Length, "", "Текст изменён; проверьте часть целиком.") };
        var audit = new { operation = "user-edit-draft", id, at = DateTimeOffset.UtcNow,
            previous, beforeText = before, currentText = after, revision };
        LiteraryChapterFiles.Write(Path.Combine(root, "Import", "review-" + Guid.NewGuid().ToString("N") + ".json"),
            JsonSerializer.Serialize(audit, ImportJson.Options));
        store.EditCompleted(id, before, after);
        LiteraryChapterFiles.Write(path, JsonSerializer.Serialize(file with { Parts = file.Parts
            .Select(p => p.PartId == id ? new ImportPartReview(id, revision, doubts) : p).ToArray() }, ImportJson.Options));
    }
